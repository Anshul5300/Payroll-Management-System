...
namespace PayrollManagementSystem_Sprint2.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> EditEmployeeDetails(int id)            //Edit Employee Details
        {
            EmployeeMaster emplist = new EmployeeMaster();
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync($"{localHostLink}api/EmployeeMaster/{id}"))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> EditEmployeeDetails(EmployeeMaster employee)
        {
            using (var httpClient = new HttpClient())
            {
                ...
                HttpResponseMessage response = httpClient.PutAsync($"{localHostLink}api/EmployeeMaster/" + employee.EmployeeId, contentData).Result;
                ViewBag.Message = response.Content.ReadAsStringAsync().Result;
            }
